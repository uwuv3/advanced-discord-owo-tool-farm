...
export const owoHandler = async (agent) => {
    agent.on("messageCreate", async (message) => {
        ...
        if (/are you a real human|(check|verify) that you are.{1,3}human!/gim.test(normalized)) {
            logger.alert(Language.__("message.captchaFoundChannel", {
            ...
            try {
                ...
                if (attachmentUrl) {
                    ...
                    if (!owo)
                        throw new Error(Language.__("fail.reachOwODmChannel"));
                    ...
                    await new Promise((resolve, reject) => {
                        const collector = owoDM.createMessageCollector({
                            filter: (msg) => msg.author.id == agent.owoID && /verified that you are.{1,3}human!/gim.test(msg.content),
                            max: 1,
                            time: 30_000
                        });
                        collector.once("end", (collection) => {
                            if (collection.size === 0)
                                reject(Language.__("fail.timedOutCaptchaAnswer"));
                    ...
                    throw new Error(Language.__("fail.noSourceDetedted"));
            ...
            logger.info(Language.__("message.captchaSolved", { next: agent.config.autoResume ? Language.__("message.restartig") : Language.__("message.exiting") }));
