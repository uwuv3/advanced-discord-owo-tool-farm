...
export const dmsHandler = async (agent: BaseAgent) => {
    agent.on("messageCreate", async (message) => {
        ...
        if(/^[a-zA-Z]{3,6}$/.test(message.content)) {
            ...
            let filter = (m:Message) => m.author.id === agent.owoID && m.channel.type == 'DM' && /(wrong verification code!)|(verified that you are.{1,3}human!)|(have been banned)/gim.test(m.content)
